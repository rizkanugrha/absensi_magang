<x-app-layout>
     ...
     <!-- Sidebar -->
        <aside class="w-64 bg-white border-r">
            ...
            <nav class="mt-6">
                ...
                <a href="{{ route('attendance.index') }}" class="flex items-center px-6 py-3 text-blue-600 bg-blue-50 font-medium">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    Attendance
                </a>
                <a href="{{ route('attendance.rekap') }}" class="flex items-center px-6 py-3 text-gray-600 hover:bg-gray-100">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h11M9 21V3m12 18V3" />
                    </svg>
                    Daily Recap
                </a>
            </nav>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-white shadow rounded-lg p-6">
                    ...
                    <div class="mt-4">
                        ...
                        <img id="preview_photo" src="" alt="Preview" class="border rounded w-full max-h-64 object-contain">
                    </div>
                </div>
                ...
                <div class="bg-white shadow rounded-lg p-6 flex flex-col justify-between">
                    <div>
                        ...
                        {{-- Form Check In --}}
                        <form id="checkinForm" action="{{ route('attendance.checkin') }}" method="POST" class="mb-6">
                            @csrf
    ...
    <script>
        ...
        document.getElementById('checkinForm').addEventListener('submit', function(e) {
            if (!lastPhoto) {
                e.preventDefault();
                alert('Please take a photo before check-in!');
            }
        });
        ...
        document.getElementById('checkoutForm').addEventListener('submit', function(e) {
            if (!lastPhoto) {
                e.preventDefault();
                alert('Please take a photo before check-out!');
                return;
            }
            if (!document.getElementById('daily_report').value.trim()) {
                e.preventDefault();
                alert('Please fill in the daily report before check-out!');
            }
        });
    </script>
</x-app-layout>
